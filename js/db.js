...
export async function search_statelike(username, postId) {
  try {
...
export async function addStateLike(username, postID) {
...
export async function deleteStateLike(username, postID) {
  const postRef = doc(db, "Post", postID);
  await updateDoc(postRef, {
    countLike: increment(-1)
  })
...
export async function search_stateBook(username, postId) {
  try {
...
export async function addStateBook(username, postID) {
...
export async function deleteStateBook(username, postID) {
  const stateRef = collection(db, 'state-bookmark');
  let q = query(stateRef, where("user", "==", username), where("post_id", "==", postID));
  let querySnapshot = await getDocs(q);
  if (!querySnapshot.empty) {
